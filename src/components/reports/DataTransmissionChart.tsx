...
export default function DataTransmissionChart({
  ...
  return (
    <div className="space-y-4 relative">
        ...
        <div className="flex items-center gap-3">
          <div className="flex flex-col">
            <h3 className="text-lg font-bold text-white">
              Data Trends
              <span className="ml-2 text-xs font-normal text-slate-500 uppercase tracking-widest px-1.5 py-0.5 bg-slate-800 rounded">
                {viewLevel} level
              </span>
            </h3>
          ...
          {activeMetric === "packets" && (
            <div className="flex items-center gap-4 px-3 py-1.5 bg-slate-950/50 rounded-lg border border-slate-800 ml-2">
              <div className="flex items-center gap-1.5">
                ...
                <span className="text-xs text-slate-300 uppercase font-bold tracking-tighter">
                  Healthy
                </span>
              </div>
              <div className="flex items-center gap-1.5">
      ...
      <div ref={chartContainerRef} className="h-[500px] w-full mt-4 relative">
        <ResponsiveContainer width="100%" height="100%">
          ...
          >
            ...
            <Legend
              ...
              iconType="circle"
              formatter={(value) => (
                <span className="text-sm text-slate-200 font-medium ml-1">
                  {String(value).replace(/_(packets|battery|temp|alerts)$/, "")}
                </span>
              )}
            />
            ...
            {entities.map((entity) => {
              ...
              const color = getDistinctColor(stableIndex >= 0 ? stableIndex : entities.indexOf(entity));
